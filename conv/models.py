...
def create_model(fingerprint_input, model_settings, model_architecture,
  ...
  elif model_architecture == 'conv_rec':
    return create_conv_rec_model(fingerprint_input, model_settings, is_training)
  else:
...
def create_conv_model(fingerprint_input, model_settings, is_training):
  ...
  final_fc_weights = tf.Variable(
      tf.truncated_normal(
          [first_fc_num, label_count], stddev=0.01))
...
def create_conv_rec_model(fingerprint_input, model_settings,
                                  is_training):
    """Builds a convolutional with LSTM layer model.
